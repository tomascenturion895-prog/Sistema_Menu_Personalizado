...
{{-- Navbar oscura estilo "terminal de codigo": fondo terminal-900 (como un editor),
     logo con cursor parpadeante y acentos en el naranja de marca --}}
<nav x-data="{ open: false }" class="bg-terminal-900 border-b border-terminal-800">
        ...
        <div class="flex justify-between h-16">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('menu.mi-pedido')" :active="request()->routeIs('menu.mi-pedido')" wire:navigate>
                        ...
                        @if (count(session('carrito', [])) > 0)
                            <span class="ml-1.5 bg-brand-500 text-white text-xs font-mono font-semibold rounded-full px-1.5">{{ count(session('carrito', [])) }}</span>
                        @endif
                    </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-terminal-300 bg-transparent hover:text-white focus:outline-none transition ease-in-out duration-150">
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-md text-terminal-300 hover:text-white hover:bg-terminal-800 focus:outline-none focus:bg-terminal-800 focus:text-white transition duration-150 ease-in-out">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-terminal-800">
            <div class="px-4">
                ...
                <div class="font-medium text-sm text-terminal-400">{{ auth()->user()->email }}</div>
            </div>
